...
/// `~/.claudexor/v3/daemon/control-api.json` (or `$CLAUDEXOR_CONFIG_DIR/daemon/...`).
public struct ControlApiDiscovery: Codable, Sendable, Equatable {
    ...
    public static func defaultPath(home: URL = FileManager.default.homeDirectoryForCurrentUser) -> URL {
        ...
        return home.appendingPathComponent(".claudexor/v3/daemon/control-api.json")
    }
...
public enum ControlApiDiscoveryError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .invalidHost(host, port):
            return "control-api discovery file carries an invalid host/port ('\(host)':\(port)) — delete ~/.claudexor/v3/daemon/control-api.json and restart the daemon"
        }
    }
}
