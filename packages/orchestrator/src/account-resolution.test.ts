...
describe("resolveAccountForRun ladder predicate (Enabled-toggle bypass fix)", () => {
  ...
  it("all-rows-disabled takes the DISCLOSED paid route under auto — never a default-store spawn", async () => {
    const context = ctx({
    ...
    const exhausted = context.events.find(
      (event) => event.type === "route.account.pool_exhausted",
    );
  ...
  it("a harness with NO registered rows keeps the legacy default-subject ladder", async () => {
    ...
    expect(context.apiKeyRouteNoted()).toBe(false);
    expect(context.events.map((event) => event.type)).not.toContain(
